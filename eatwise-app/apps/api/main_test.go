...
func TestShoppingUpdateAndPurchasedTogglePersistsInDB(t *testing.T) {
	requireDB(t)
	userID, token := setupTestUser(t)
	householdID := setupTestHousehold(t, userID)
	defer cleanupTestData(t, userID)
	...
	err := db.QueryRow(`
		INSERT INTO shopping_list (household_id, user_id, name, quantity, unit, category, purchased)
		VALUES ($1, $2, 'Milk', 1, 'liter', 'Dairy', false)
		RETURNING id
	`, householdID, userID).Scan(&itemID)
	if err != nil {
		t.Fatalf("Failed to seed shopping item: %v", err)
	...
	// Update full editable fields and set purchased=true.
	updateReq := map[string]interface{}{
		"name":      "Organic Milk",
		"quantity":  3,
		"unit":      "bottles",
	...
	var purchased bool
	err = db.QueryRow(`
		SELECT name, quantity, unit, category, purchased, purchased_at
		FROM shopping_list
		WHERE id = $1
	`, itemID).Scan(&name, &quantity, &unit, &category, &purchased, &purchasedAt)
	if err != nil {
		t.Fatalf("Failed to verify updated shopping item: %v", err)
	}

	if name != "Organic Milk" || quantity != 3 || unit != "bottles" || category != "Groceries" || !purchased {
		t.Fatalf("Unexpected DB state after update: name=%q quantity=%d unit=%q category=%q purchased=%v", name, quantity, unit, category, purchased)
	}
	if !purchasedAt.Valid {
		t.Fatal("Expected purchased_at to be set when purchased=true")
	}

	// Toggle purchased back to false and ensure purchased_at is cleared.
	toggleBody := []byte(`{"purchased":false}`)
	toggleReq := httptest.NewRequest("PUT", "/api/shopping-list/"+strconv.Itoa(itemID), bytes.NewReader(toggleBody))
	...
	updateShoppingItemHandler(toggleRes, toggleReq)

	if toggleRes.Code != http.StatusOK {
		t.Fatalf("Expected status 200 when toggling purchased false, got %d", toggleRes.Code)
	}
	...
	err = db.QueryRow(`
		...
		FROM shopping_list
		WHERE id = $1
	`, itemID).Scan(&purchased, &purchasedAt)
	if err != nil {
		t.Fatalf("Failed to verify toggled shopping item: %v", err)
	}

	if purchased {
		t.Fatal("Expected purchased=false after toggle")
	}
	if purchasedAt.Valid {
		t.Fatal("Expected purchased_at to be NULL after toggling purchased=false")
	}
}
